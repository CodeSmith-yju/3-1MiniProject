...
public class TalkMgr : MonoBehaviour // 대화 데이터를 관리할 매니저 스크립트
{
    ...
    private void GenerateTalkData()// 오브젝트들의 상호작용 대사를 만들어서 스크립트 실행할때 호출되게 함
    {
        ...
        dictTalkData.Add(70 + 1000, new string[] {
            ...
            "안녕하십니까, " + GameMgr.playerData[0].GetPlayerName() + " 모험가님. \n급히 전해드릴 소식이 있습니다. :0",
            "최근 던전 포탈에서 이상 현상이 감지되어, \n새로운 미지의 던전으로 연결되는 통로가 발견되었습니다. :0",
            "'상급 모험가'로서의 책임을 다하여 \n미지의 던전을 탐사해주시길 부탁드립니다. :0",
            "던전에 진입하기 전, \n충분히 준비를 갖추신 후 도전해 주시기 바랍니다. :0",
            "모험가님의 무사 귀환을 기원하겠습니다. :0"
        });
        dictTalkData.Add(71 + 1000, new string[] {
            //"71, 1000여기는이제 상급 던전에 대한 설명 핵심 다시알려주는거, 상점이용추천하는거 :0"
            "어떤 적들이 출몰할지 모르니 조심하세요. :0"
        });
        dictTalkData.Add(70 + 2000, new string[] {
            //"70, 2000, 여기서 80으로 jump :0"
            "귀환을 환영합니다, " + GameMgr.playerData[0].GetPlayerName() + " 모험가님. :0",
            "모험가님께서 탐험하신 그 던전은 \n이제 '악마의 미궁'이라 불리게 되었습니다. :0",
            "모험가님의 용맹과 희생은 길드뿐만 아니라, \n모든 모험가들에게 영감과 희망을 안겨주었습니다. :0",
            "이번 탐험의 성공으로 인해 모험가님은 '특급 모험가'로 승급됩니다. \n그 업적은 영원히 기려질 것입니다. :0",
            "모험가님의 헌신과 노력에 깊은 감사를 드리며, \n길드의 모든 구성원이 축하의 마음을 전합니다. :0",
        });

        dictTalkData.Add(80 + 1000, new string[] {
            GameMgr.playerData[0].GetPlayerName()+"님 오셨군요. :0"
        });
        dictTalkData.Add(81 + 1000, new string[] {
            "81, 1000, 여기서 80으로 jump :0"
        });
        dictTalkData.Add(80 + 2000, new string[] {
            GameMgr.playerData[0].GetPlayerName()+"님 오셨군요. :0"
        });
        /*dictTalkData.Add(40 + 1000, new string[] {
